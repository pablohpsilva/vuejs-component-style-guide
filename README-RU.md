...
В Vue тег компонента является кастомным HTML-тегом который может использоваться как корневой элемент CSS стилей (`vertical-slider .list-element`). Либо, другой вариант, имя компонента может быть использовано как префикс CSS класса всех стилей компонента (`.vertical-slider .list-element`).
...
В этом README файле можно описать функционал модуля и варианты использования. Для компонентов Vue самым полезным является описать атрибуты так как именно они являются выражением API компонента.
